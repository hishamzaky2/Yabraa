...
protocol DismissHandler: class {
    func sheetViewControllerDismissed()
}
...
public class SheetViewController: UIViewController {
    ...
    weak var delegate: DismissHandler?
    ...
    public convenience init(controller: UIViewController, sizes: [SheetSize] = []) {
        ...
        self.modalPresentationStyle = .overCurrentContext //.overFullScreen
    }
    ...
    private func setUpContainerView() {
        self.view.addSubview(self.containerView) { (subview) in
            ...
            var inset = self.height(for: self.containerSize) == UIScreen.main.bounds.height ? 0 : self.safeAreaInsets.top + 20
            
            subview.top.pinToSuperview(inset: 0, relation: .greaterThanOrEqual) //self.safeAreaInsets.top + 20
            ...
            if self.adjustForBottomSafeArea {
                self.containerHeightConstraint.constant += bottomInset
            }
        }
    ...
    private func setUpChildViewController() {
        ...
        self.containerView.addSubview(self.childViewController.view) { (subview) in
            ...
            subview.top.pinToSuperview(inset: 0, relation: .equal) //24
        }
...
//        handleView.backgroundColor = self.handleColor
    ...
    @objc func dismissTapped() {
        ...
        self.closeSheet {
            self.delegate?.sheetViewControllerDismissed()
        }
    }
...
extension SheetViewController: UIGestureRecognizerDelegate {
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if velocity.y < 0 {
            ...
            return height(for: self.orderedSheetSizes.last) > containerHeight && containerHeight < height(for: SheetSize.allScreen)
        } else {
            return true
        }
    }
